...
import logoName from '../assets/trama-name.png';
...
export function Sidebar({ isExpanded, setIsExpanded }: Props) {
  ...
  return (
    <aside
      className={`${isExpanded ? 'w-72' : 'w-18'} overflow-hidden fixed max-h-screen h-full flex flex-col bg-white border-r border-neutral-200 shadow-sm transition-all duration-300`}
    >
      <ul className="mt-8 text-neutral-600 text-[15px] flex flex-col h-full">
        <li className="pl-6 pb-6 flex items-center gap-4 border-b border-neutral-300">
          <img width={22} height={22} src="/trama-icon.png" draggable={false} />
          ...
          <img
            ...
            src={logoName}
            draggable={false}
            className={`${isExpanded ? 'opacity-100' : 'opacity-0'} transition-all duration-300`}
          />
        </li>
        ...
        <li>
          <button
            ...
            className="flex w-full pl-6 py-3 gap-4 truncate hover:text-black"
          >
        ...
        <li className="mt-auto">
          <button
            type="button"
            onClick={signOut}
            className="flex w-full pl-6 py-3 gap-4 truncate text-red-500 hover:text-red-700"
          >
